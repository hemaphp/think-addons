...
class Service extends \think\Service
{
    ...
    {
        if (!$name || is_dir(app()->addons->getAddonsPath() . $name)) {
            return ['msg' => '插件已经存在'];
        }
    ...
    public static function uninstall($name, $force = true)
    {
        ...
        @rmdirs(app()->addons->getAddonsPath() . $name);// 移除插件目录
        Service::refresh();// 刷新
        return true;
    }
    ...
    {
        $dir = app()->addons->getAddonsPath() . $name . DIRECTORY_SEPARATOR;
        return $dir;
    }
    /**
     * 获取插件备份目录
    ...
    {
        // 初始化插件目录
        $addons_path = root_path() . 'addons' . DIRECTORY_SEPARATOR;
        // 如果插件目录不存在则创建
